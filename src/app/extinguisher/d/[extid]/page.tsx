...
export default function ExtinguisherDetailPage({ params }: { params: Promise<{ extid: string }> }) {
    ...
    const [isReqUseToken, setIsReqUseToken] = useState(true);
    ...
    const extBody = (
        <div className={`min-h-screen bg-gray-50 flex flex-col items-center px-6 ${isReqUseToken ? 'pt-24' : ''}`}>
            {!isReqUseToken && (
                <div className="w-full max-w-sm md:max-w-full flex flex-col items-center mb-6 pt-6 pb-6 bg-purple-600 rounded-b-3xl">
                    <h1 className="text-xl font-bold text-white mb-1">Informasi APAR</h1>
                    <p className="text-sm text-white text-center">Powered by NEXA (PT Tan Anugerah Sejahtera)</p>
                </div>
            )}
    ...
    return (
        isReqUseToken
            ? <MainLayout appBarTitle='APAR' showNavBar={false}>{extBody}</MainLayout>
            : extBody
    )
}